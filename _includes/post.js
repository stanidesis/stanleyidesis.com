window.addDependencyCallback(function() {
  if (!window.matchMedia || !window.matchMedia("(pointer: coarse)").matches) {
    /* Use selection share only on desktop (non-mobile) */
    loadjs(
      [
        "/js/highlight-share.js",
        "/js/sharers/copy.js",
        "/js/sharers/email.js",
        "/js/sharers/facebook.js",
        "/js/sharers/linked-in.js",
        "/js/sharers/reddit.js",
...
function postPostProcessing() {
  if ($("aside.post-nav").css("display") == "none") {
    ...
    $("div.post-nav-content")
      .detach()
      .appendTo("div.post-mobile-nav-content-wrapper");
    ...
    var toggleMobileNav = function(event) {
      ...
      if (isGaAvailable()) {
        ga(
          "send",
          "event",
          "{{site.ga.categories.resource}}",
          "{{site.ga.actions.click}}",
          mobileNavActive
            ? "{{site.ga.labels.hide-nav}}"
            : "{{site.ga.labels.show-nav}}"
        );
      }
  ...
  loadjs("/js/social-share-media.min.js", function() {
    ...
    $shareDropdown
      .find("a.post-nav-action-dropdown-share-facebook")
      .attr("href", socialmediaurls["facebook"]);
    $shareDropdown
      .find("a.post-nav-action-dropdown-share-twitter")
      .attr("href", socialmediaurls["twitter"]);
    $shareDropdown
      .find("a.post-nav-action-dropdown-share-linkedin")
      .attr("href", socialmediaurls["linkedin"]);
    $shareDropdown
      .find("a.post-nav-action-dropdown-share-email")
      .attr("href", socialmediaurls["email"]);
  });
  ...
  audio.onended = function() {
    if (isGaAvailable()) {
      ga(
        "send",
        "event",
        "{{site.ga.categories.playback-resource}}",
        "{{site.ga.actions.click}}",
        "{{site.ga.labels.finished}}"
      );
    }
    updateControlsToPaused();
  };
  $(SEL_PLAY_PAUSE_WRAPPER)
    .find(SEL_PLAY_PAUSE_CLICK)
    .first()
...
const SEL_PLAY_PAUSE_WRAPPER =
  "div.post-nav-playback-action-wrapper-play-pause";
